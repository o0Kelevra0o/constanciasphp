document.addEventListener("DOMContentLoaded", function () {
  ...
  var fechaHora = new Date().toLocaleString("es-MX", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
    hour: "2-digit",
  ...
  // Generar folio
  document
    .getElementById("generar_folio_btn")
    .addEventListener("click", function () {
      ...
      xhrConsecutivo.open(
        "GET",
        "conexion.php?accion=obtener_ultimo_consecutivo&tabla=" + tabla,
        true
      );
      xhrConsecutivo.send();
    });
  ...
  document.getElementById("guardar_btn").addEventListener("click", function () {
    ...
    var sql =
      "INSERT INTO " +
      tabla +
      " (Folio, Ape_pat, Ape_Mat, Nombres, Expediente, hora_llegada, hora_salida, Elaboro) " +
      "VALUES ('" +
      folio +
      "', '" +
      ape_pat +
  ...
  // Agregar evento de clic al botón "Imprimir"
  document
    .getElementById("imprimir_btn")
    .addEventListener("click", function () {
      // Obtener los datos del formulario
      const folio = document.getElementById("folio_label").textContent;
      ...
      // Definimos el fondo
      doc.addImage("/img/fondo.jpg", "JPEG", 0, 0, 210, 297); // Ajustar tamaño según necesidades

      // Agregamos los datos al PDF
      doc.setFontSize(12);
      doc.text(`Folio: ${folio}`, 20, 80);
      doc.text(`Fecha: ${fecha}`, 20, 90);
      doc.text("A quien corresponda:", 20, 100);
      doc.text(
        `Por medio de la presente, se hace constar que el (la) C. ${nombreCompleto}, con No. de Expediente: ${expediente}, asistió el día de hoy a atención médica en la ${unidad}, entre las ${horaLlegada} y las ${horaSalida}.`,
        20,
        110
      );
      doc.text(`${elaboro}`, 20, 150); // Ajustar posición según necesidades

      // Mostramos el documento PDF en una nueva ventana
